...
const VerifyUser = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    console.log(otpInfo);
    try {
      const response = await axios.post('http://138.201.123.93:8000/user/validate/', otpInfo);
      // Handle successful registration
      console.log(response);
      if(response.status === 200) navigate('/login');
      if(response.status === 404) alert("Input correct email!");
      if(response.status === 400) alert("Invalidated email!");
    } catch (error) {
  ...
  return (
    <div className='flex flex-col gap-3 m-auto w-[20rem] min-h-[10rem] border-1 border-slate-900 rounded-xl backdrop-blur-sm bg-black/30 p-10'>
        <p className='text-white text-[2rem]'>Input OTPcode:</p>
        ...
        <button className='w-full h-11 bg-gradient-to-r from-fuchsia-500 to-purple-500 text-white font-bold font-mono rounded-lg' onClick={ handleSubmit}>Submit</button>
